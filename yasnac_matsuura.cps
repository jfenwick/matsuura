...
// matsuura
var nOutput = createVariable({prefix:"N"}, nFormat);
...
function onOpen() {
  ...
  if (programName) {
    ...
    var oFormat = createFormat({width:5, zeropad:true, decimals:000});
    //if (programComment) {
  ...
  */
  ...
  // Standard stuff Matsuura
  // Matsuura - Absolute position, stop canned cycle, cutter comp off, hard coded feed output for start
  writeBlock(gAbsIncModal.format(90), "G80", "G40", gMotionModal.format(1), feedOutput.format(250));
  // Matsuura -  Go to 0 at H00 (no tool)
  //writeBlock(gMotionModal.format(1), xOutput.format(0), yOutput.format(0), zOutput.format(0), hFormat.format(0));
  writeBlock(gFormat.format(1), xOutput.format(0), yOutput.format(0), zOutput.format(0), hFormat.format(0));
  // Compulsory stop
  //writeBlock("M00");
  /*
  switch (unit) {
...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    // Matsuura Pick tool, turn on spindle
    writeBlock(
  ...
  if (workOffset > 0) {
    ...
    // matsuura removed work offset stuff
    /*
  ...
  if (insertToolCall || retracted) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      //writeBlock(gFormat.format(1), zOutput.format(initialPosition.z), hFormat.format(lengthOffset));
      writeBlock(gFormat.format(1), zOutput.format(initialPosition.z), hFormat.format(lengthOffset));
    } else {
      // Matsuura commented out
      /*
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
        gFormat.format(43), xOutput.format(Math.round(initialPosition.x * 10000)),
        yOutput.format(Math.round(initialPosition.y * 10000)),
        zOutput.format(Math.round(initialPosition.z * 10000)), hFormat.format(lengthOffset)
      );
      */
    }
...
function onClose() {
  ...
  // matsuura removed
  //writeBlock(gFormat.format(28), gAbsIncModal.format(91), "Z" + xyzFormat.format(0)); // retract
  writeBlock(gFormat.format(0), zOutput.format(0), hFormat.format(0));
  zOutput.reset();
